...
* **Mode toggle**:
  ...
  * `QUIZ_MODE=live` → fetch via the external quiz API. Requires `QUIZ_API_AUTH` and optionally `QUIZ_API_BASE`, `QUIZ_LENGTH` (5–10). League is selected dynamically from available leagues. Failing upstream responses return `502` with a generic error and are logged without PII.
